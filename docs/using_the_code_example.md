...
The following example clones the "[Audio recorder using emUSB-device](https://github.com/Infineon/mtb-example-psoc-edge-usb-device-audio-recorder)" application with the desired name "AudioRecorder" configured for the *KIT_PSE84_EVAL_EPC2* BSP into the specified working directory, *C:/mtb_projects*:

   ```
   project-creator-cli --board-id KIT_PSE84_EVAL_EPC2 --app-id mtb-example-psoc-edge-usb-device-audio-recorder --user-app-name AudioRecorder --target-dir "C:/mtb_projects"
   ```
